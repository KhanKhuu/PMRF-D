...
//--------------------------------------------------------------------------------------------------------------------
//expects an empty vector<vector<pair<uint8_t, int>>> and a vector<uint8_t> containing the data as its input.
//finds the first neighborhood and the first neighborhood of all of the first neighbors, storing the pixel
//	values and positions in pairs.
//the algoritm finds the first neighborhoods assuming a 2 dimensional image of dimensions WIDTH x HEIGHT, and
//	it stores them in 1 dimensional fashion. ICM will use these values according to the order they were stored,
//	assuming contiguous elements are also nearby elements in a smooth image.
//the algorithm stores these values according to the order in which it finds them, which is the same order every time.
//---------------------------------------------------------------------------------------------------------------------
void LAP (std::vector<std::vector<std::pair<uint8_t, int>>> *cliques, 
	...
	for (int i = 0; i < HEIGHT * WIDTH; i++) {
		...
		get_first_neighborhood (data, &clique, WIDTH, HEIGHT, i);		
		...
		if (i % WIDTH) { //i % WIDTH is 0 on the left edges
			get_first_neighborhood (data, &clique, WIDTH, HEIGHT, i - 1);
		}
		...
		if (i >= WIDTH) { //top neighbor exists for every element after those in the first row
			get_first_neighborhood (data, &clique, WIDTH, HEIGHT, i - WIDTH);
		}
